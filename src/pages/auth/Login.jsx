...
export default function Login() {
  ...
  const validar = () => {
    ...
    else if (form.password.length < 4) e.password = 'La contraseña es muy corta'
    setErrores(e)
  ...
  const inp = (field) => ({
    style: {
      ...
      fontSize: '14px', outline: 'none', boxSizing: 'border-box', transition: 'border-color .2s'
    },
    ...
    <div style={{ minHeight: '100vh', background: '#0a0a0a', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '20px' }}>
      ...
      <div className="fade-in" style={{ width: '100%', maxWidth: '420px', position: 'relative' }}>
        <div style={{ textAlign: 'center', marginBottom: '40px' }}>
          ...
          <h1 style={{ fontFamily: 'Poppins, sans-serif', fontSize: '30px', fontWeight: '800', color: '#f0f0f0', letterSpacing: '-0.5px', lineHeight: 1.1 }}>Fast & Healthy</h1>
        ...
        <div style={{ background: '#111', border: '1px solid #2a0000', borderRadius: '20px', padding: '36px', boxShadow: '0 0 40px rgba(200,0,0,0.1)' }}>
          <h2 style={{ fontFamily: 'Poppins, sans-serif', fontSize: '20px', fontWeight: '700', marginBottom: '24px', color: '#f0f0f0' }}>Iniciar Sesión</h2>
          ...
          <form onSubmit={handleSubmit} noValidate autoComplete="off" style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
            ...
            <div>
              <label style={{ display: 'block', fontSize: '13px', color: '#aaa', marginBottom: '8px', fontWeight: '500' }}>Correo electrónico</label>
              <input
                type="email" required
                value={form.email}
            ...
            <div>
              <label style={{ display: 'block', fontSize: '13px', color: '#aaa', marginBottom: '8px', fontWeight: '500' }}>Contraseña</label>
              <div style={{ position: 'relative' }}>
                <input
                  type={verClave ? 'text' : 'password'} required
                ...
                />
                <button type="button" onClick={() => setVerClave(!verClave)} style={{ position: 'absolute', right: '12px', top: '50%', transform: 'translateY(-50%)', background: 'none', border: 'none', cursor: 'pointer', fontSize: '18px', color: '#666', padding: '4px' }}>
                  {verClave ? '🙈' : '👁️'}
                </button>
              </div>
        ...
        <p style={{ textAlign: 'center', color: '#555', fontSize: '12px', marginTop: '24px' }}>Fast & Healthy © 2026</p>
      </div>
    </div>
  )
}
