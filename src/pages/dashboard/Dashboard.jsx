    ...
    <div className="fade-in">
      ...
      <div style={{ marginBottom: '32px' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: '12px', marginBottom: '6px' }}>
          ...
          <span style={{ color: 'var(--muted)', fontSize: '13px' }}>Sistema activo</span>
        </div>
        <h1 style={{ fontFamily: 'Poppins, sans-serif', fontSize: '30px', fontWeight: '800', color: 'var(--text)' }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '20px' }}>
        ...
        <Card>
          <div style={{ padding: '20px', borderBottom: '1px solid var(--border)' }}>
            <h3 style={{ fontFamily: 'Poppins, sans-serif', fontSize: '16px', fontWeight: '700', color: 'var(--text)' }}>Pedidos activos</h3>
          </div>
          <div style={{ padding: '8px' }}>
            {pedidos.length === 0 ? (
              <div style={{ padding: '24px', textAlign: 'center', color: 'var(--muted)', fontSize: '14px' }}>No hay pedidos activos</div>
              ...
              <div key={p.id} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '10px 14px', borderRadius: '10px' }}>
                <div>
                  <div style={{ fontSize: '13px', fontWeight: '600', color: 'var(--text)' }}>{p.numeroPedido}</div>
                  <div style={{ fontSize: '11px', color: 'var(--muted)', marginTop: '2px' }}>{p.mesa || p.tipoPedido}</div>
                </div>
        ...
        <Card>
          <div style={{ padding: '20px', borderBottom: '1px solid var(--border)' }}>
            <h3 style={{ fontFamily: 'Poppins, sans-serif', fontSize: '16px', fontWeight: '700', color: 'var(--text)' }}>⚠️ Alertas de stock</h3>
          </div>
          <div style={{ padding: '8px' }}>
            {stockBajo.length === 0 ? (
              <div style={{ padding: '24px', textAlign: 'center', color: 'var(--muted)', fontSize: '14px' }}>✅ Todo el inventario OK</div>
              ...
              <div key={i.id} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '10px 14px', borderRadius: '10px' }}>
                <div style={{ fontSize: '13px', color: 'var(--text)', fontWeight: '500' }}>{i.nombreIngrediente}</div>
                <div style={{ textAlign: 'right' }}>
                  <span style={{ fontSize: '12px', color: i.stockActual <= i.stockMinimo ? '#fca5a5' : '#fbbf24', fontWeight: '600' }}>
                    {i.stockActual} {i.unidadMedida}
                  </span>
                  <div style={{ fontSize: '10px', color: 'var(--muted)' }}>mín: {i.stockMinimo}</div>
                </div>
              </div>
            ))}
          </div>
